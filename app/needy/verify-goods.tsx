         ...
         setNeedyDetails(prev => {
           ...
           return {
             ...
             goods_of_registre: list.map((g: any) => ({
               ...
               GoodID: Number(g.GoodID ?? g.id ?? 0),
               Verified: g.Verified ?? g.verified ?? false
             }))
           };
         });
       } else {
...
const sendSms = async (goodId: string) => {
  ...
  try {
    const response = await apiService.sendSMSCode(needyDetails.Phone, goodId);
    if (response.success) {
        router.push(`/needy/verify-sms?goodId=${goodId}`);
    } else {
                  ...
                  <View style={styles.bottomToggleContainer}>
                    <TouchableOpacity
                      style={[
                        ...
                        good.Verified ? styles.bottomToggleActive : styles.bottomToggleInactive,
                      ]}
                      onPress={() => sendSms(good.GoodID)}
                    >
                      <ThemedText style={styles.bottomToggleText}>
                        {good.Verified ? '✅ تأیید شده' : 'تأیید نشده'}
                      </ThemedText>
                    </TouchableOpacity>
                  </View>
                     <View>
