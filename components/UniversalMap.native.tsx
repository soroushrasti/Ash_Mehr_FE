...
export default function UniversalMap({ location, onLocationSelect, mapType = 'standard', zoom = 0.05, showControls = true }: UniversalMapProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setMapsError('react-native-maps is not available in this build');
    }
  }, []);
  ...
  if (!MapsMod) {
    return (
      <View style={{ width: '100%', height: 300, borderRadius: 12, marginBottom: 16, alignItems: 'center', justifyContent: 'center', backgroundColor: '#f2f2f2', padding: 12 }}>
